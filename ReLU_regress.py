...
def main():
  ...
  epsilon    = 0.0001 # step fraction
  reg_lambda = 0.01 # regularization strength
 
  def step(xval):
    yval = 0.5*(np.sign(xval) + 1)
    yval[xval == 0] = 0
    return yval
  ...
  def calculate_loss(model):
    ...
    a1 = np.fmax(np.zeros(z1.shape),z1)
    z2 = a1.dot(W2) + b2
    yhat = Z2
    
    #Calculate loss function for regression
    data_loss = 0.5 * np.dot((yhat-y).T,(yhat-y))
  ...
  def build_model(nn_hdim, num_passes=20000, print_loss=False):
    ...
    for i in range(0, num_passes):
      ...
      delta2 = delta3.dot(W2.T) * step(z1)
  ...
  model = build_model(100, num_passes=20001, print_loss=True)
